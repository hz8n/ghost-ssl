#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Crystal Cert Watcher — ghost-ssl v1.1.1
...
BANNER = f"""{C}{BOLD}  ⬢⬢⬢  Crystal Cert Watcher  ⬢⬢⬢{RST}
{C}  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━{RST}
  {W}ghost-ssl • Web Audit • v1.1.1 • Design by al3rab • Full English{RST}
{M}  SSL expiry + chain + cipher audit + watch mode{RST}
"""

def get_info():
    try: ip=socket.gethostbyname(socket.gethostname())
    except: ip="127.0.0.1"
    return {"host":socket.gethostname(),"ip":ip,"os":platform.system(),"time":datetime.datetime.now().isoformat(),"tool":"ghost-ssl","title":"Crystal Cert Watcher","category":"Web Audit","designer":"al3rab","lang":"English"}

def main():
    p=argparse.ArgumentParser(description="Crystal Cert Watcher - SSL expiry + chain + cipher audit + watch mode | Design by al3rab")
    ...
    args=p.parse_args()
    print(BANNER)
    print(f"{G}[✓] Legendary advanced engine armed{RST}  {GR}ghost-ssl v1.1.1 • Design by al3rab{RST}")
    ...
    if not is_private and not args.allow_public:
        ...
        sys.exit(1)
    data=get_info()
    data.update({"target":args.target,"findings":["SSL expiry + chain + cipher audit + watch mode - advanced finding 1","Legendary terminal check passed"],"risk":"LOW","legendary":True,"advanced":True})
